...
4. **Version bump**: update the version string in both
   `pyproject.toml` (the `version` field under `[project]`) and
   `src/mcp_signal/server.py` (the `version=` argument to `FastMCP`),
...
6. **Report**: print the new version, release URL, and a one-line summary.
